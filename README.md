...
本项目采用**双重许可**模式。默认情况下，Tsumugi Pay 按 [GNU Affero General Public License v3.0（AGPL-3.0）](LICENSE) 提供；完整许可文本位于仓库根目录的 `LICENSE` 文件。任何复制、修改、分发或通过网络向用户提供修改版本的行为，均须遵守 AGPL-3.0 的全部条件，包括其相应源代码提供义务。

如果您的商业使用计划**无法或不希望遵守 AGPL-3.0 的义务**，例如将本项目整合到闭源产品、以不披露相应源代码的方式向用户提供网络服务，您必须在使用前向版权持有人购买单独的**付费商业授权**。商业授权的范围、费用及其他条款须以另行签署的书面授权协议为准。请通过 [WindyPear-Team/tsumugi-pay](https://github.com/WindyPear-Team/tsumugi-pay) 联系项目维护者洽谈商业授权。

> AGPL-3.0 本身不禁止商业活动；在完全遵守 AGPL-3.0 的前提下进行商业使用，不需要额外取得商业授权。付费商业授权适用于希望获得不受 AGPL-3.0 相应开源义务约束的权利的使用者。
...
演示账户仅在显式设置 `BOOTSTRAP_DEMO=true` 的本地开发环境中创建，方便验证后台界面。账号为 `admin@tsumugi.local`，密码为 `ChangeMe123!`。默认值为 `false`，且生产环境拒绝启动任何启用演示引导的配置。
...
| `BOOTSTRAP_DEMO` | `false` | 必须为 `false`；生产环境拒绝启用该选项。 |
...
| 商户回调 SSRF 防护 | 后台“站点”设置默认启用该防护。它会阻断环回、私网、链路本地、组播、未指定地址和管理员填写的额外 CIDR，并在重定向和实际连接时再次校验目标；仅在明确了解风险时才可关闭。 |
