export type View = "overview" | "bills" | "refunds" | "channels" | "audit" | "developers" | "users" | "settings"
...
export type ManagedUser = AccountUser & { account?: { name: string; merchant_no: string } }
...
export type PublicSiteConfig = { site_name: string; allow_password_login: boolean; allow_password_registration: boolean; email_whitelist: string[]; terms_url: string; privacy_policy_url: string }
export type SiteSettings = { site: PublicSiteConfig; email: { host: string; port: number; username: string; from: string; password_configured: boolean }; hcaptcha: { enabled: boolean; site_key: string; secret_key_configured: boolean }; oidc: { enabled: boolean; issuer_url: string; client_id: string; redirect_url: string; authorization_endpoint: string; token_endpoint: string; jwks_uri: string; client_secret_configured: boolean } }
