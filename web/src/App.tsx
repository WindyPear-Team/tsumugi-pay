  ...
  async function loadInitial() {
    ...
    try {
      ...
      if (me.role === "platform_admin") {
		...
		setTenantId(me.account_id);
		if (me.account) setTenants([me.account]);
      }
...
function Setup({ onCompleted }: { onCompleted: (email: string) => void }) {
  ...
  async function submit(event: FormEvent) { event.preventDefault(); if (password !== confirmPassword) { setError("两次输入的密码不一致"); return; } setBusy(true); setError(""); try { const response = await fetch(`${API}/api/v1/setup/initialize`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ account_name: accountName, merchant_no: merchantNo, display_name: displayName, email, password }) }); const body = await response.json().catch(() => ({})); if (!response.ok) throw new Error(body.message || "初始化失败"); setCreated({ merchantNo: body.account.merchant_no, apiSecret: body.credentials.api_secret, callbackSecret: body.credentials.callback_secret }); } catch (e: any) { setError(e.message); } finally { setBusy(false); } }
  if (created) return <div className="login-page"><div className="login-panel"><div className="brand login-brand"><span className="brand-mark">T</span><span>Tsumugi <b>Pay</b></span></div><div className="login-copy"><p className="eyebrow">ACCOUNT CREATED</p><h1>用户已创建。</h1><p>请立即保存以下开放支付凭据；为安全起见，它们不会再次显示。</p></div><div className="credential-box"><Detail label="商户号" value={created.merchantNo}/><Detail label="API 请求密钥" value={created.apiSecret}/><Detail label="回调签名密钥" value={created.callbackSecret}/></div><button className="primary-button full" onClick={() => onCompleted(email)}>我已安全保存，前往登录 <span>→</span></button></div><div className="login-art"><div className="orb orb-one"/><div className="orb orb-two"/><p>从你的账户开始，<br/>接入每一笔交易。</p></div></div>;
  return <div className="login-page"><div className="login-panel"><div className="brand login-brand"><span className="brand-mark">T</span><span>Tsumugi <b>Pay</b></span></div><div className="login-copy"><p className="eyebrow">FIRST-RUN SETUP</p><h1>创建你的<br/><em>用户账户。</em></h1><p>每个用户拥有独立的支付工作区，可自行添加支付宝或微信支付通道。</p></div><form onSubmit={submit}><Label>账户名称<Input value={accountName} onChange={(e) => setAccountName(e.target.value)} autoComplete="organization" required /></Label><Label>商户号（留空自动生成）<Input value={merchantNo} onChange={(e) => setMerchantNo(e.target.value)} /></Label><Label>姓名<Input value={displayName} onChange={(e) => setDisplayName(e.target.value)} autoComplete="name" required /></Label><Label>邮箱<Input value={email} onChange={(e) => setEmail(e.target.value)} type="email" autoComplete="email" required /></Label><Label>设置密码<Input value={password} onChange={(e) => setPassword(e.target.value)} type="password" minLength={10} autoComplete="new-password" required /></Label><Label>确认密码<Input value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} type="password" minLength={10} autoComplete="new-password" required /></Label>{error && <p className="form-error">{error}</p>}<button className="primary-button full" disabled={busy}>{busy ? "创建中…" : "创建用户并继续"} <span>→</span></button></form><p className="demo-note">初始化入口只会在系统尚无用户时开放一次。</p></div><div className="login-art"><div className="orb orb-one"/><div className="orb orb-two"/><div className="transaction-card"><span>SECURE SETUP / STEP 1</span><b>用户初始化</b><small>创建你的独立支付工作区</small></div><p>从你的账户开始，<br/>建立可信的支付运营体系。</p></div></div>;
}
...
function Channels({ data, tenant, canManage, onRefresh, request }: { data: Channel[]; tenant?: Tenant; canManage: boolean; onRefresh: () => void; request: (path: string, options?: RequestInit) => Promise<any> }) {
  const [editing, setEditing] = useState<Channel | null>(null); const [adding, setAdding] = useState(false);
  if (!tenant) return <div className="content"><Empty icon="◫" title="当前用户账户不可用" copy="请重新登录后再配置支付通道。" /></div>;
  return <div className="content"><section className="tenant-banner"><div><p className="eyebrow">当前用户账户</p><h3>{tenant.name}</h3><span>商户号 {tenant.merchant_no}</span></div><div><span className={`status ${tenant.status}`}>{statusName[tenant.status]}</span><small>支付密钥不会在界面回显</small>{canManage && <button className="primary-button" onClick={() => setAdding(true)}>新增通道 <span>＋</span></button>}</div></section><section className="channel-grid">{data.map((channel) => <article className="channel-card" key={channel.id}><div className="channel-card-top"><span className={`large-provider ${channel.provider}`}>{channel.provider === "alipay" ? "支" : "微"}</span><span className={`status ${channel.enabled ? "paid" : "pending"}`}>{channel.enabled ? "已启用" : "未启用"}</span></div><h3>{channel.display_name}</h3><p>{channel.provider === "alipay" ? "网页支付与手机网站支付，使用 RSA2 签名和异步通知验签。" : "Native、H5 场景，使用微信支付 API v3 请求签名与通知解密。"}</p><div className="channel-meta"><span><i className={channel.configured ? "green" : "gray"}/> {channel.configured ? "已保存凭据" : "尚未配置"}</span>{canManage && <button className="text-button" onClick={() => setEditing(channel)}>配置通道 →</button>}</div></article>)}</section>{!data.length && <Empty icon="◈" title="尚未添加支付通道" copy="添加支付宝或微信支付通道后，在可视化表单中填写官方凭据即可开始收款。" />}<section className="panel webhook-panel"><div><div><p className="eyebrow">WEBHOOK</p><h3>官方异步通知</h3><p>将每个通道对应的回调 URL 填入服务商控制台。验签、幂等处理和商户通知重试均由后端完成。</p></div><span className="webhook-lock">● 已启用签名验真</span></div></section>{adding && <AddChannelModal onClose={() => setAdding(false)} onSaved={() => { setAdding(false); onRefresh(); }} request={request}/>} {editing && <ChannelModal channel={editing} onClose={() => setEditing(null)} onSaved={() => { setEditing(null); onRefresh(); }} request={request}/>}</div>;
}

function AddChannelModal({ onClose, onSaved, request }: { onClose: () => void; onSaved: () => void; request: (path: string, options?: RequestInit) => Promise<any> }) { const [provider, setProvider] = useState("alipay"); const [name, setName] = useState("支付宝"); const [error, setError] = useState(""); const [saving, setSaving] = useState(false); async function save() { setSaving(true); setError(""); try { await request("/api/v1/admin/channels", { method: "POST", body: JSON.stringify({ provider, display_name: name }) }); onSaved(); } catch (e: any) { setError(e.message); } finally { setSaving(false); } } return <Modal title="新增支付通道" onClose={onClose}><p className="modal-intro">每位用户可独立接入支付宝和微信支付；添加后再填写对应官方凭据。</p><Label className="form-label">支付服务商<Select value={provider} onValueChange={(value) => { if (value) { setProvider(value); setName(value === "alipay" ? "支付宝" : "微信支付"); } }}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="alipay">支付宝</SelectItem><SelectItem value="wechat">微信支付</SelectItem></SelectContent></Select></Label><Label className="form-label">展示名称<Input value={name} onChange={(e) => setName(e.target.value)} /></Label>{error && <p className="form-error">{error}</p>}<button className="primary-button full" onClick={save} disabled={saving}>{saving ? "添加中…" : "添加并配置"}</button></Modal>; }
...
function ChannelModal({ channel, onClose, onSaved, request }: { channel: Channel; onClose: () => void; onSaved: () => void; request: (path: string, options?: RequestInit) => Promise<any> }) {
  const [name, setName] = useState(channel.display_name); const [enabled, setEnabled] = useState(channel.enabled); const [fields, setFields] = useState<Record<string, string>>({}); const [error, setError] = useState(""); const [saving, setSaving] = useState(false); const setField = (key: string, value: string) => setFields((current) => ({ ...current, [key]: value })); const isAlipay = channel.provider === "alipay";
  async function save() { setSaving(true); setError(""); const config = isAlipay ? { alipay: { app_id: fields.app_id || "", app_private_key_pem: fields.app_private_key_pem || "", alipay_public_key_pem: fields.alipay_public_key_pem || "", gateway_url: fields.gateway_url || "", return_url: fields.return_url || "" } } : { wechat: { mch_id: fields.mch_id || "", app_id: fields.app_id || "", merchant_serial_no: fields.merchant_serial_no || "", merchant_private_key_pem: fields.merchant_private_key_pem || "", api_v3_key: fields.api_v3_key || "", platform_public_key_pem: fields.platform_public_key_pem || "", platform_serial_no: fields.platform_serial_no || "" } }; const body = { display_name: name, enabled, ...(Object.keys(fields).length ? { config } : {}) }; try { await request(`/api/v1/admin/channels/${channel.id}`, { method: "PATCH", body: JSON.stringify(body) }); onSaved(); } catch (e: any) { setError(e.message); } finally { setSaving(false); } }
  return <Modal title={`配置${providerName[channel.provider]}`} onClose={onClose}><p className="modal-intro">凭据以 AES-GCM 加密方式保存且不会回显；重新保存时请填写完整官方凭据。</p><Label className="form-label">展示名称<Input value={name} onChange={(e) => setName(e.target.value)}/></Label>{isAlipay ? <><Label className="form-label">App ID<Input value={fields.app_id || ""} onChange={(e) => setField("app_id", e.target.value)} /></Label><Label className="form-label">应用私钥（PEM）<Textarea value={fields.app_private_key_pem || ""} onChange={(e) => setField("app_private_key_pem", e.target.value)} rows={5}/></Label><Label className="form-label">支付宝公钥（PEM）<Textarea value={fields.alipay_public_key_pem || ""} onChange={(e) => setField("alipay_public_key_pem", e.target.value)} rows={5}/></Label><Label className="form-label">网关地址（可选）<Input placeholder="https://openapi.alipay.com/gateway.do" value={fields.gateway_url || ""} onChange={(e) => setField("gateway_url", e.target.value)} /></Label><Label className="form-label">同步跳转地址（可选）<Input value={fields.return_url || ""} onChange={(e) => setField("return_url", e.target.value)} /></Label></> : <><Label className="form-label">微信支付商户号<Input value={fields.mch_id || ""} onChange={(e) => setField("mch_id", e.target.value)} /></Label><Label className="form-label">应用 AppID<Input value={fields.app_id || ""} onChange={(e) => setField("app_id", e.target.value)} /></Label><Label className="form-label">商户证书序列号<Input value={fields.merchant_serial_no || ""} onChange={(e) => setField("merchant_serial_no", e.target.value)} /></Label><Label className="form-label">商户私钥（PEM）<Textarea value={fields.merchant_private_key_pem || ""} onChange={(e) => setField("merchant_private_key_pem", e.target.value)} rows={5}/></Label><Label className="form-label">API v3 密钥<Input type="password" value={fields.api_v3_key || ""} onChange={(e) => setField("api_v3_key", e.target.value)} /></Label><Label className="form-label">平台公钥（PEM）<Textarea value={fields.platform_public_key_pem || ""} onChange={(e) => setField("platform_public_key_pem", e.target.value)} rows={5}/></Label><Label className="form-label">平台证书序列号（可选）<Input value={fields.platform_serial_no || ""} onChange={(e) => setField("platform_serial_no", e.target.value)} /></Label></>}<div className="webhook-copy"><b>回调地址</b><code>{channel.webhook_url}</code></div><div className="switch"><Switch checked={enabled} onCheckedChange={setEnabled}/><span />完成配置后启用此通道</div>{error && <p className="form-error">{error}</p>}<button className="primary-button full" onClick={save} disabled={saving}>{saving ? "保存中…" : "安全保存通道配置"} <span>→</span></button></Modal>;
}
...
function Settings({ user, tenant }: { user: any; tenant?: Tenant }) { return <div className="content"><section className="settings-grid"><article className="panel"><p className="eyebrow">CURRENT SESSION</p><h3>登录身份</h3><Detail label="邮箱" value={user?.email || "—"}/><Detail label="权限" value={roleName(user?.role || "")}/><Detail label="当前账户" value={tenant?.name || "—"}/></article><article className="panel"><p className="eyebrow">SECURITY BASELINE</p><h3>安全基线</h3><ul className="check-list"><li><i>✓</i> 通道密钥 AES-GCM 加密存储</li><li><i>✓</i> JWT 用户数据隔离</li><li><i>✓</i> 支付回调官方签名校验</li><li><i>✓</i> 账单状态机及通知幂等</li><li><i>✓</i> 操作审计日志</li></ul></article></section></div> }
...
function titleFor(view: View) { return ({ overview: "工作台", bills: "账单中心", refunds: "退款管理", channels: "支付通道", tenants: "用户管理", users: "成员与权限", audit: "审计日志", developers: "开发者接入", settings: "系统设置" })[view] }
function roleName(role: string) { return ({ platform_admin: "系统用户", tenant_admin: "账户所有者", tenant_operator: "支付操作员", tenant_viewer: "只读成员" })[role] || role }
