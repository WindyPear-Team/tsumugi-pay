...
import { AuditPage, DevelopersPage, RefundsPage, SettingsPage } from "@/pages/ManagementPages"
import { LoginPage, SetupPage } from "@/pages/AuthPages"
import { DashboardPage } from "@/pages/DashboardPage"
import { BillsPage } from "@/pages/BillsPage"
import { ChannelsPage } from "@/pages/ChannelsPage"
import type { ApiRequest, AuditLog, Bill, Channel, Dashboard, Refund, SiteSettings, Account, View } from "@/types"

const viewPaths: Record<View, string> = { overview: "/", bills: "/bills", refunds: "/refunds", channels: "/channels", audit: "/audit-logs", developers: "/developers", settings: "/settings" }
const titles: Record<View, string> = { overview: "工作台", bills: "账单中心", refunds: "退款管理", channels: "支付通道", audit: "审计日志", developers: "开发者接入", settings: "站点设置" }
...
export default function App() {
  ...
  async function loadPage() {
    ...
    try {
      ...
      if (view === "settings") setSiteSettings(await request("/api/v1/admin/site-settings"))
  ...
  if (!token) return <LoginPage initialEmail={setupEmail} onSuccess={(value) => { localStorage.setItem("tsumugi_access_token", value); setToken(value); navigate("/", { replace: true }) }} />
  if (location.pathname === "/login" || location.pathname === "/setup") return <Navigate to="/" replace />
  const nav: { key: View; label: string; icon: string }[] = [{ key: "overview", label: "工作台", icon: "⌘" }, { key: "bills", label: "账单中心", icon: "▤" }, { key: "refunds", label: "退款管理", icon: "↩" }, { key: "channels", label: "支付通道", icon: "◈" }, { key: "audit", label: "审计日志", icon: "☷" }, { key: "developers", label: "开发者接入", icon: "</>" }, { key: "settings", label: "站点设置", icon: "⚙" }]
  return <SidebarProvider><Sidebar className="sidebar" collapsible="icon"><div className="brand"><span className="brand-mark">T</span><span>Tsumugi <b>Pay</b></span></div><div className="workspace-label">支付运营中心</div><SidebarContent><SidebarMenu>{nav.map((item) => <SidebarMenuItem key={item.key}><SidebarMenuButton className="nav-item" isActive={view === item.key} onClick={() => navigate(viewPaths[item.key])}><span className="nav-icon">{item.icon}</span>{item.label}</SidebarMenuButton></SidebarMenuItem>)}</SidebarMenu></SidebarContent><SidebarFooter><button className="profile" onClick={logout}><span className="avatar">{(user?.email || "A")[0].toUpperCase()}</span><span><b>{user?.email || "用户"}</b><small>退出登录</small></span></button></SidebarFooter></Sidebar><main className="main"><header className="topbar"><div><p className="eyebrow">TSUMUGI PAY / {view.toUpperCase()}</p><h1>{titles[view]}</h1></div><div className="top-actions"><SidebarTrigger className="mobile-sidebar-trigger"/><button className="icon-button" onClick={loadPage} aria-label="刷新页面">↻</button></div></header>{error && <div className="alert"><b>操作提示：</b>{error}</div>}{loading && <div className="loading-line"/>}<Routes><Route path="/" element={<DashboardPage data={dashboard} bills={bills} navigate={(next) => navigate(viewPaths[next])}/>}/><Route path="/bills" element={<BillsPage data={bills} canManage={canManagePayments} request={request} onRefresh={loadPage}/>}/><Route path="/refunds" element={<RefundsPage data={refunds}/>}/><Route path="/channels" element={<ChannelsPage data={channels} account={activeAccount} canManage={canManagePayments} request={request} onRefresh={loadPage}/>}/><Route path="/audit-logs" element={<AuditPage data={auditLogs}/>}/><Route path="/developers" element={<DevelopersPage account={activeAccount}/>}/><Route path="/settings" element={<SettingsPage user={user} account={activeAccount} settings={siteSettings} request={request} onSaved={setSiteSettings}/>}/><Route path="*" element={<Navigate to="/" replace/>}/></Routes></main></SidebarProvider>
}
