...
import { AuditPage, DevelopersPage, ProfilePage, RefundsPage, SettingsPage, UsersPage } from "@/pages/ManagementPages"
...
function viewFor(pathname: string): View {
  return (Object.entries(viewPaths).find(([, path]) => path === pathname)?.[0] as View | undefined) || "overview"
}
...
export default function App() {
  ...
  function logout() {
    localStorage.removeItem("tsumugi_access_token")
    setToken("")
    setUser(null)
    setAccount(null)
  }
  ...
  async function loadPage() {
    ...
    try {
      if (view === "overview") {
        const [summary, list] = await Promise.all([request("/api/v1/admin/dashboard"), request("/api/v1/admin/bills")])
  ...
  useEffect(() => {
    if (siteConfig) document.title = `${pageTitle} - ${siteConfig.site_name}`
  }, [pageTitle, siteConfig])
  useEffect(() => {
    document.documentElement.dataset.theme = theme
    localStorage.setItem("tsumugi_theme", theme)
  }, [theme])
  useEffect(() => {
    if (!siteConfig) return
    document.documentElement.style.setProperty("--theme-color", siteConfig.theme_color || "#2f9c84")
    if (siteConfig.favicon_url) {
      ...
      icon.href = siteConfig.favicon_url
    }
  }, [siteConfig])
  useEffect(() => {
    const oidcToken = new URLSearchParams(location.search).get("oidc_token")
    if (!token && oidcToken) {
      localStorage.setItem("tsumugi_access_token", oidcToken)
      setToken(oidcToken)
  ...
  useEffect(() => {
    if (token) loadInitial()
      ...
      fetch(`${API}/api/v1/setup/status`)
        .then((response) => (response.ok ? response.json() : { required: false }))
        .then((body) => setSetupRequired(Boolean(body.required)))
    ...
    return (
      <div className="login-page">
        <div className="login-panel">
          <p className="demo-note">正在检查系统初始化状态…</p>
        </div>
      </div>
    )
  ...
  if (!token)
    return (
      <LoginPage
        ...
        site={siteConfig}
        onSuccess={(value) => {
          localStorage.setItem("tsumugi_access_token", value)
          setToken(value)
          navigate("/", { replace: true })
        }}
      />
  ...
  const nav: { key: View; label: string; icon: string }[] = [
    ...
    ...(isPlatformAdmin
      ? [
          { key: "users" as View, label: "用户管理", icon: "♙" },
          { key: "settings" as View, label: "站点设置", icon: "⚙" },
        ]
      : []),
  ]
  return (
    <SidebarProvider>
      <Sidebar className="sidebar" collapsible="icon">
        <div className="brand">
          ...
          <span>{siteConfig?.site_name || "Tsumugi Pay"}</span>
        </div>
          ...
          <SidebarMenu>
            {nav.map((item) => (
              <SidebarMenuItem key={item.key}>
                <SidebarMenuButton className="nav-item" isActive={view === item.key} onClick={() => navigate(viewPaths[item.key])}>
        ...
        <SidebarFooter>
          <button className="theme-toggle" type="button" title={theme === "dark" ? "切换亮色" : "切换暗色"} onClick={() => setTheme(theme === "dark" ? "light" : "dark")}>
          ...
          <button className="profile" onClick={logout}>
            ...
            <span>
              <b>{user?.username || user?.email || "用户"}</b>
              <small>退出登录</small>
            </span>
          </button>
        </SidebarFooter>
      ...
      <main className="main">
        <header className="topbar">
          <div>
            <p className="eyebrow">
              {siteConfig?.site_name || "TSUMUGI PAY"} / {view.toUpperCase()}
            </p>
            <h1>{titles[view]}</h1>
          </div>
          <div className="top-actions">
            <SidebarTrigger className="mobile-sidebar-trigger" />
            <button className="icon-button" onClick={loadPage} aria-label="刷新页面">
              ↻
            </button>
          </div>
        </header>
        ...
        <Routes>
          <Route path="/" element={<DashboardPage data={dashboard} bills={bills} navigate={(next) => navigate(viewPaths[next])} />} />
          <Route path="/bills" element={<BillsPage data={bills} canManage={canManagePayments || isPlatformAdmin} request={request} onRefresh={loadPage} />} />
              ...
              isPlatformAdmin ? (
                <SettingsPage
                  ...
                  onSaved={(saved) => {
                    ...
                    setSiteConfig({
                      ...saved.site,
                      oidc_enabled: saved.oidc.enabled,
                      oidc_login_label: saved.oidc.login_label,
                    })
                  }}
                />
              ) : (
